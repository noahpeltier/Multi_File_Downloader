import-Module $PSScriptRoot\PoshProgressBar -force
...
function Start-Download {
    ...
    Begin {
        ...
        $Library = "$PSScriptRoot\lib"
        $Progressbar = New-ProgressBar -Size Large -MaterialDesign -Theme Dark -IconPath $Library\icon.ico
    ...
    Process {
        ...
        while ((Get-BitsTransfer | Where-Object { $_.JobState -eq "Transferring" }).Count -gt 0) {     
            ...
            if ($totalbytes -gt 0) {              
                ...
                # Formatting the output into the XAML window This part gets REALLY messy with
                Write-ProgressBar `
                    ...
                    -Status (
                        Get-BitsTransfer | select `
                            ...
                            @{n = 'Percent Complete'; e = {"$([math]::round(($_.Bytestransferred * 100) / $_.bytestotal))".PadLeft(3, ' ') + "%:$((Show-bar -transferred $_.Bytestransferred -Total $_.bytestotal))"} } |
...
Export-ModuleMember Get-DellDriverPack,
    Get-OptiplexDrivers,
    Get-LatitudeDrivers,
    Get-WinPEDrivers,
    Get-DellBIOS,
    New-BitsDownloadList,
    Show-Bar,
    Start-Download